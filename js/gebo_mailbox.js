/* [ ---- Gebo Admin Panel - mailbox ---- ] */
    ...
    gebo_mailbox = {
		...
		actions: function() {
			...
			$('.sel_msgs').on('click', function () {
				var tableid = $(this).closest('table').data('tableid');
				$('#'+tableid).find('input[name=sel_msg]').each(function(){
					$(this).prop("checked",!!$(".sel_msgs").prop("checked"));
				});
				if($(this).is(':checked')) {
